...
do
  ...
  do
	...
	echo -n "${TRIAL}, " >> ${RESULTS_FILE}
        ...
        FIXED_IMAGE=$(ls ${PREFIX}0.0*pertr${TRIAL}_*.rfd)
        MOVING_IMAGE=$(ls ${PREFIX}${STRAINS[index]}*pertr${TRIAL}_*.rfd)
	...
	echo -n "${REG}, " >> ${RESULTS_FILE}
        ${ITK_STATISTICS} -x 0.1 -y 0.3 \
        ${OUTPUT_PREFIX_FULL}*_StrainComponent2.mha | ${STRAIN_LINE_PY} n >> ${RESULTS_FILE} || die "Measuring statistics on StrainComponent2 failed."
        ${ITK_STATISTICS} -x 0.1 -y 0.3 \
        ${OUTPUT_PREFIX_FULL}*_StrainComponent0.mha | ${STRAIN_LINE_PY} >> ${RESULTS_FILE} || die "Measuring statistics on StrainComponent0 failed."
        let 'TRIAL++'
      done
    done
