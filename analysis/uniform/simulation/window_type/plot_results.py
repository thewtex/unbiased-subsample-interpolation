...
window_types = ['blackman', 'cosine', 'hamming', 'lanczos', 'welch']
...
class PlotResults( object ):
    ...
    def _plot_snre_curve( self, curve, curve_name, reg, reg_name, snre_record,
        ...
        strains, mean_snre, snre_error = get_snre( window_types, snre_record, reg,
                filename_creator )

        self.iden_index = iden_index_start
        strains_of_interest_idxs = [1,2]
        for idx in strains_of_interest_idxs:
            plt.errorbar( np.arange( len( window_types )), mean_snre[:,idx], snre_error[:,idx],
            ...
            plt.gca().set_xticks( np.arange( len( window_types )))
            plt.gca().set_xticklabels( [w.capitalize() for w in window_types] )
            ...
            #plt.ylim( (10**-2, 10**2 ) )
            plt.ylim( (10**-0.3, 10**0.8 ) )
            plt.xlim( (-0.5, len(window_types)-0.5) )
            self.iden_index += 1
...
def filename_creator( rad ):
    return 'SNRe_' + rad + '_sim.csv'
iden_index_start = 1
plot_results.plot_snre_curves( 'radius_sim', 'Simulation', filename_creator )
def filename_creator( rad ):
    return '../../phantom/window_type/SNRe_' + rad + '.csv'
iden_index_start = 3
plot_results.plot_snre_curves( 'radius', 'Phantom', filename_creator )


plt.figure( 1 )
plt.savefig( '../../../../doc/images/window_type_regularization_axial.png' )
plt.savefig( '../../../../doc/images/window_type_regularization_axial.eps' )
plt.figure( 2 )
plt.savefig( '../../../../doc/images/window_type_regularization_lateral.png' )
plt.savefig( '../../../../doc/images/window_type_regularization_lateral.eps' )
plt.figure( 3 )
plt.savefig( '../../../../doc/images/window_type_no_regularization_axial.png' )
plt.savefig( '../../../../doc/images/window_type_no_regularization_axial.eps' )
plt.figure( 4 )
plt.savefig( '../../../../doc/images/window_type_no_regularization_lateral.png' )
plt.savefig( '../../../../doc/images/window_type_no_regularization_lateral.eps' )
